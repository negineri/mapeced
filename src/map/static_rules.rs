...
///
/// 静的ルールプロファイルはルール取得元と CE IPv6 アドレス計算方式を束ねる。
///
/// | プロファイル | ルール取得元                  | CE IPv6 計算方式         |
/// |------------|------------------------------|------------------------|
/// | `v6plus`   | `assets/v6plus_rules.json`   | v6プラス方式             |
/// | `ocn_vc`   | `assets/ocn_vc_rules.json`   | RFC 7597 標準方式        |
/// | `dhcpv6`   | DHCPv6 キャプチャ             | RFC 7597 標準方式        |
#[derive(Debug, Clone, PartialEq, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum MapProfile {
    /// v6プラス対応サービス向け（JPIX 規格）。
    V6plus,
    /// OCN バーチャルコネクト向け（JPNE 規格、RFC 7597 準拠）。
    OcnVc,
