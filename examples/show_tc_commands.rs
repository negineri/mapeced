...
///
/// 使い方:
///   cargo run --example show_tc_commands
///   cargo run --example show_tc_commands -- --psid 0xab --a-min 1
///
...
use mapeced::nftables::manager::TcManager;
...
fn main() {
    ...
    let cmds = TcManager::generate_tc_commands(&params, &iface);

    println!(
        "# tc commands  iface={iface}  psid=0x{psid:02x}  a_min={a_min}  ({} commands)\n",
        cmds.len()
    );
    for cmd in &cmds {
        println!("{cmd}");
