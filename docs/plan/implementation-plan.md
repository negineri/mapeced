...
# 代わりに u32 フィルタで宛先 IPv4 アドレスと PSID ビットを直接照合する。
# TCP（proto=6）
tc filter add dev <tunnel_iface> parent ffff: protocol ip u32 \
    match ip dst <ce_ipv4>/32 \
    match ip protocol 6 0xff \
    match u16 <psid_val> <psid_check_mask> at nexthdr+2 \
    action pedit ...
# UDP（proto=17）
tc filter add dev <tunnel_iface> parent ffff: protocol ip u32 \
    match ip dst <ce_ipv4>/32 \
...
1. tc ingress の u32 フィルタで PSID ビット照合（フィルタがマッチしたパケットに pedit を適用）
