...
- 複数プレフィックスが含まれる場合は先頭の 1 件を使用する（スペースで `split_whitespace` して最初の要素）。`/128` など単一ホストアドレスは除外し、`/` を含む CIDR 表記のみを対象とする
...
table ip mapeced {
    chain postrouting {
        ...
        # SNAT: 連続ポートレンジ（PORT_START–PORT_END）を割り当て、tc pedit のために fwmark を付与
...
# fwmark == 1 のパケット（SNAT 済み = 送信）に pedit を適用
...
/// `MapRule::is_fmr == true` のルールが存在する場合のみ `add_fmr_route` でその IPv4 プレフィックスへの FMR ルートを追加する
pub async fn apply(
...
///      新 IPv6 アドレス付与。CE IPv4 も変化するため IPv4 アドレス差し替え + nftables + tc 再適用も必ず実施する
///   2. PSID 変化（CE IPv6 変化なし）→ nftables + tc を再適用（CE IPv4 も変化するため IPv4 アドレス差し替えも実施）
///   3. BR アドレス変化のみ（CE IPv6・PSID に変化なし）→ `update_tunnel_remote` で remote エンドポイントを in-place 更新
///   4. 変化なし → 何もしない
pub async fn update(
    state: &mut DaemonState,
    new_params: MapeParams,
